import { createRequire as __workjetCreateRequire } from 'node:module'; const require = __workjetCreateRequire(import.meta.url); // Workjet Pi sidecar; @earendil-works/pi-agent-core 0.80.2; generated, do not edit.
...
import { posix as path2 } from "node:path";
...
function FromArray14(root, path3, current, next) {
  if (!guard_exports.IsArray(current)) {
    ...
    for (let index = 0; index < next.length; index++) {
      FromValue5(root, `${path3}/${index}`, current[index], next[index]);
    }
...
function FromObject17(root, path3, current, next) {
  if (!guard_exports.IsObjectNotArray(current)) {
    ...
    for (const nextKey of nextKeys) {
      AssertKey(nextKey);
      FromValue5(root, `${path3}/${nextKey}`, current[nextKey], next[nextKey]);
    }
  }
}
...
function FromValue5(root, path3, current, next) {
  ...
  if (guard_exports.IsObject(next))
    return FromObject17(root, path3, current, next);
...
function* FromObject18(path3, left, right) {
  if (!guard_exports.IsObject(right) || guard_exports.IsArray(right))
    return yield CreateUpdate(path3, right);
  ...
  for (const key of rightKeys) {
    ...
    if (guard_exports.IsUnsafePropertyKey(key))
      continue;
    yield CreateInsert(`${path3}/${key}`, right[key]);
  }
  for (const key of leftKeys) {
    ...
    if (Equal(left, right))
      continue;
    yield* FromValue6(`${path3}/${key}`, left[key], right[key]);
  }
  for (const key of leftKeys) {
    if (guard_exports.HasPropertyKey(right, key))
      continue;
...
function* FromArray15(path3, left, right) {
  if (!guard_exports.IsArray(right))
    return yield CreateUpdate(path3, right);
  for (let i = 0; i < Math.min(left.length, right.length); i++) {
    yield* FromValue6(`${path3}/${i}`, left[i], right[i]);
  }
  for (let i = 0; i < right.length; i++) {
    if (i < left.length)
      continue;
    yield CreateInsert(`${path3}/${i}`, right[i]);
  }
...
function* FromTypedArray2(path3, left, right) {
  ...
  if (predicate) {
    for (let index = 0; index < Math.min(left.length, right.length); index++) {
      yield* FromValue6(`${path3}/${index}`, left[index], right[index]);
    }
  } else {
    return yield CreateUpdate(path3, right);
  }
}
function* FromUnknown3(path3, left, right) {
  if (left === right)
    return;
  yield CreateUpdate(path3, right);
}
function* FromValue6(path3, left, right) {
  return globals_exports.IsTypeArray(left) ? yield* FromTypedArray2(path3, left, right) : guard_exports.IsArray(left) ? yield* FromArray15(path3, left, right) : guard_exports.IsObject(left) ? yield* FromObject18(path3, left, right) : yield* FromUnknown3(path3, left, right);
}
...
// src/pi-ai-compat-shim.ts
function validateToolArguments(tool, toolCall) {
  const converted = value_exports.Convert(tool.parameters, structuredClone(toolCall.arguments));
  if (value_exports.Check(tool.parameters, converted)) return converted;
  const errors = [...value_exports.Errors(tool.parameters, converted)].map((error) => error.message).join("; ");
...
async function executeToolCallsParallel(currentContext, assistantMessage, toolCalls, config, signal, emit) {
  ...
  const orderedFinalizedCalls = await Promise.all(finalizedCalls.map((entry2) => typeof entry2 === "function" ? entry2() : Promise.resolve(entry2)));
  const messages = [];
...
var Alpha2 = class extends APIResource {
  constructor() {
    super(...arguments);
...
var AssistantMessageEventStream = class extends EventStream3 {
  constructor() {
...
function getBunSandboxEnvValue(name) {
  ...
  if (procEnvCache === null) {
    ...
    try {
      ...
      for (const entry2 of data.split("\0")) {
        const idx = entry2.indexOf("=");
        if (idx > 0) {
          procEnvCache.set(entry2.slice(0, idx), entry2.slice(idx + 1));
        }
      }
    } catch {
    }
...
function convertResponsesMessages(model, context, allowedToolCallProviders, options) {
  ...
  for (const msg of transformedMessages) {
    if (msg.role === "user") {
      ...
      const textResult2 = msg.content.filter((c) => c.type === "text").map((c) => c.text).join("\n");
      const hasImages = msg.content.some((c) => c.type === "image");
      const hasText = textResult2.length > 0;
      ...
      if (hasImages && model.input.includes("image")) {
        ...
        if (hasText) {
          contentParts.push({
            type: "input_text",
            text: sanitizeSurrogates(textResult2)
          });
        }
        for (const block of msg.content) {
          if (block.type === "image") {
        ...
        output = sanitizeSurrogates(hasText ? textResult2 : "(see attached image)");
      }
      messages.push({
...
async function processResponsesStream(openaiStream, output, stream2, model, options) {
  ...
  for await (const event of openaiStream) {
    if (event.type === "response.created") {
      ...
      finalizeResponse2(event.response);
    } else if (event.type === "error") {
...
var MAX_ASSISTANT_TURNS = 24;
var MemoryWorkspace = class {
  ...
  snapshot() {
    return [...this.files.entries()].sort(([left], [right]) => left.localeCompare(right)).map(([filePath, content]) => ({ path: filePath, content }));
  }
};
var readSchema = typebox_exports.Object({ path: typebox_exports.String() });
var writeSchema = typebox_exports.Object({ path: typebox_exports.String(), content: typebox_exports.String() });
var listSchema = typebox_exports.Object({ prefix: typebox_exports.Optional(typebox_exports.String()) });
var grepSchema = typebox_exports.Object({ pattern: typebox_exports.String(), prefix: typebox_exports.Optional(typebox_exports.String()) });
function toolsFor(workspace) {
  return [
    {
      name: "read",
      label: "read",
      ...
      async execute(_id, input) {
        ...
        if (content === void 0) return textResult(`File not found: ${filePath}`, true);
...
function normalizePath(value) {
  ...
  const normalized = path2.normalize(`/${value}`).slice(1);
...
function gatewayModel(input) {
  ...
  return {
    ...
    contextWindow: input.contextWindow ?? 128e3,
...
async function handleRequest(request, streamFn) {
  ...
  try {
    ...
    const prompt = { role: "user", content: request.prompt, timestamp: Date.now() };
    ...
    const messages = await runAgentLoop(
      ...
      void 0,
      streamFn ?? ((selectedModel, selectedContext, options) => stream(selectedModel, selectedContext, options))
    );
...
function startOneShotServer(socketPath) {
...
async function dispatchLine(line, socket, server, socketPath) {
  let response;
  try {
...
function installCleanup(server, socketPath) {
  const cleanup = () => {
    server.close();
    rmSync(socketPath, { force: true });
  };
...
var entry = process.argv[1] ? pathToFileURL(process.argv[1]).href : "";
