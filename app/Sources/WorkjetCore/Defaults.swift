    ...
    **Update duty stays EVENT-driven** (worker lands / decision taken / finding
    ...
    public static func configuration() -> WorkjetConfiguration {
        ...
        return WorkjetConfiguration(
            ...
            - Never infer checkout/worktree isolation from a path printed by a worker. Current remote repository runs use Workjet's imported workspace. A local run uses the checkout from which `workjet run` was invoked; do not run two local writing workers against that checkout concurrently.
