    ...
    **Update duty stays EVENT-driven** (worker lands / decision taken / finding
    ...
    public static func configuration() -> WorkjetConfiguration {
        ...
        return WorkjetConfiguration(
            ...
            - Repository workers run only in Workjet-owned isolated worktrees created from an immutable snapshot of the invoking checkout. Initialized non-recursive Git submodules are materialized offline at their pinned commits inside that worktree and are read-only for result capture. Never infer a different workspace identity from a path printed by a worker, and never edit the caller checkout on a worker's behalf.
            ...
            Work only in the checkout supplied by Workjet and never cd to another checkout. Repository-backed local and remote runs use Workjet-owned isolated worktrees created from immutable snapshots; initialized non-recursive Git submodules are available offline at their pinned commits and must remain unchanged. Never edit the invoking checkout directly. Commit in green slices so a timeout can still land value. No subagents.
